...
<img width="1920" height="918" alt="изображение" src="https://github.com/user-attachments/assets/e60adeb3-8f64-4951-989a-273d32f2aca2" />
...
1. Какой IP-адрес и номер порта TCP использует ваш компьютер (отправитель), передающий
   ...
   - мой: 192.168.0.112:46622 потом 60632
   - <img width="1920" height="918" alt="изображение" src="https://github.com/user-attachments/assets/662e240d-948b-4e1f-8f94-4d1485bb67fd" />

2. Каков IP-адрес у сервера gaia.cs.umass.edu? Каковы номера портов для отправки и приема
   TCP-сегментов этого соединения?
   - 149.154.175.54, порт 443, потом 80
   - <!-- todo -->
3. Какой порядковый номер у SYN TCP-сегмента, который используется для установления
   ...
   <img width="1920" height="918" alt="изображение" src="https://github.com/user-attachments/assets/6b2fe079-a72a-4c7d-a32f-bcfd4048b13b" />
...
5. Какой порядковый номер у SYNACK-сегмента, отправленного сервером gaia.cs.umass.edu
   ...
   - номер ноль, во флагах 0x012: это единички в нужных позициях
   - <!-- todo -->
   - <!-- todo -->
   - <!-- todo -->
6. Какой порядковый номер у TCP-сегмента, содержащего команду POST протокола HTTP?
   ...
   - номер 1
   <img width="1920" height="918" alt="изображение" src="https://github.com/user-attachments/assets/b3539859-f947-44c5-a363-68ed34e95373" />
...
7. Рассмотрите TCP-сегмент, содержащий команду POST протокола HTTP, как первый TCP-сегмент 
   ...
   - sequence numbers: 1, 2072, 4968, 7864, 10760, 15104
   - 5.247260237, 5.248412300,  5.255631769, 5.256576115, 5.257308467, 5.384604850
   - 5.248407551, 5.255627541, 5.256572208, 5.257304941, 5.258076276, 5.387073702
   - Разницы: 0.001147314, 0.007215241, 0.000940439, 0.000728826, 0.000767809, 0.002468852, это и есть RTT.
     <img width="1836" height="740" alt="изображение" src="https://github.com/user-attachments/assets/933e6ab4-3b8f-4941-88c1-9f74e07898cd" />
...
8. Чему равна пропускная способность (количество байтов, передаваемых в единицу
   ...
   - Надо найти количество байтов и количество времени передачи. Для этого надо посмотреть на первый POST запрос и на последний ACK пакет, вычесть и это и будет то что надо. Для количества байтов надо посмотреть на seq number последнего ACK пакета, разделить одно на другое. Итак, количество байтов получилось 152945 - 1 = 152944 байт, время конца 5.956967507, время начала помним 5.247260237. Тогда пропускная способность получается 152944 / (5.956967507 -  5.247260237) = 215502.9 байт, что пример 216 килобайт в секунду.
     
     <img width="1836" height="740" alt="изображение" src="https://github.com/user-attachments/assets/abdc1472-d0c5-4a90-9476-109af6af46ed" />
     <img width="1836" height="740" alt="изображение" src="https://github.com/user-attachments/assets/e3d5336a-1d0d-44d5-8304-7075750c3398" />
...
#### Решение
хотим, чтобы окна принимающего и отдающего не пересекались: иначе коллизия и отсутствие взаимопонимания будет. Тогда наш ответ -- половина от объема диапазона (если больше, то сервер все принял, отправил ACK, ничего не дошло до отправителя, хотя бы один пакет имеет тот же номер и коллизия, запутались, а половины хватает, так как коллизий не случится)
...
#### Решение
посчитаем по формуле, решение найдем через вольфрам альфа.
$$\dfrac{nL/R}{RTT + L/R} > 0.98 \rightarrow \dfrac{n1500 * 8 / 10^9}{0.03 + 1500 * 8 / 10^9} > 0.98$$
выходит, $n >= 2451$.
