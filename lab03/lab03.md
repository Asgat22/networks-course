...
#### Решение
Время с момента создания бита до декодирования есть общая задержка. Она складывается из задержки распространения, времени передачи, и времени на преобразование сигнала (в обе стороны, поэтому его берем два раза). Тогда общее время получится
$$2 * 56 \cdot 8 / 128 + 56 * 8 / 1000 + 5 = 7 + 0.448 + 5 = 12.448$$
...
#### Решение
Из условия $N = 10 + 1 = 11$. задержки ожидания и передачи равны по 10 мс из условия, тогда $d = 10 + 10 = 20$ мс. Тогда $a = 11 / 0.02 = 550$ пакетов в секунду. 
...
#### Решение
1. Так как узким местом является первая линия, второму пакету не придется ждать между двумя линиями. Тогда разница во времени, в окторое они придут будет время, за которое второй пакет проходит первую часть маршрута, а это $L / R_S$.

2. Да, второй пакет может находиться в буфере ожидая передачи во вторую линию, если отправится через время, меньшее, чем $L/R_C - L/R_S$. Для удобства время, через которое отправлен второй пакет после отправки первого обозначу за $t$. Тогда первый дошел до буфера через $t + L/R_S$ времени после начала, если это меньше, чем $L/R_C$, то мы ждем, иначе нет.  
### Задача 4 (4 балла)
...
#### Решение
1. $\Delta = 850000b / 15Mbps = 850000b / 15000000bps = 17 / 300  s$
2. Из условия маршрутизатор ждет 3 сек, а оставшаяся задержка считается по формуле. $\Delta$ знаем из п. 1,  $B = 16$ по условию. Тогда ответ равен $3 + (17/300)/(1 - 17 * 16/300) = 3 + (17/300)/((75 - 17 * 4) / 75) = 3 + 17 / (4 * 7) = 3 + 17/28$ сек
3. Мы с вер. 0.6 работаем за $850000b/100Mbps$, и с вер. 0.4 за число из п.2. Тогда наш ответ это $0.6 * 8500/1000000  + 0.4(3 + 17/28) = 0.0051 + 1.2 + 17 / 70 = 1.2051 + 17/70$ секунд, что примерно равно 1.4479.
