...
#### Демонстрация работы
<img width="588" height="211" alt="изображение" src="https://github.com/user-attachments/assets/06bb25ba-1b91-4b96-ae71-92978c1949a7" />
...
#### Демонстрация работы
<img width="684" height="183" alt="изображение" src="https://github.com/user-attachments/assets/509a53b4-75e7-4f8b-b2e1-95b72f8d72c1" />
...
#### Решение
Окно растет линейно от $\dfrac{W}{2}$ до $W$, то есть всего мы передали $\dfrac{W}{2} + \dfrac{W}{2} + 1 + \ldots + W = \dfrac{3W}{2} \cdot (\dfrac{W}{2} + 1) \cdot \dfrac{1}{2} = \dfrac{3W^2 + 6W}{8}$

Из этих всех пакетов мы потеряли только один, а значит частота потери равна $L = \dfrac{1}{\frac{3W^2 + 6W}{8}} = \dfrac{8}{3W^2 + 6W} = \dfrac{1}{\frac{3}{8}W^2 + \frac{3}{4}W}$, что и требовалось.

Оценим среднюю скорость. Она равна $\dfrac{MSS}{RTT} \cdot w$, где $w$ есть среднее количество переданных пакетов, оно равно $\dfrac{\frac{W}{2} + W}{2} = \dfrac{3W}{4}$. Оценим это через $L$. Из формулы выше: $\dfrac{1}{\sqrt{L}} \approx \sqrt{\frac{8}{3}W}$. Тогда $w \approx 0.75 \cdot \sqrt{\dfrac{8}{3}} \dfrac{1}{\sqrt{L}} \approx \dfrac{1.22}{\sqrt{L}}$, а тогда скорость как раз примерно равна $\dfrac{1.22MSS}{RTT \sqrt{L}}$
...
#### Решение
Пусть мы сделали $n$ увеличений, тогда $a^n \approx 2$, начинали с $\frac{W}{2}$, тогда всего мы отправили $\frac{W}{2}(1 + a + a^2 + \ldots + a^{n}) = \frac{W}{2}\dfrac{a^{n + 1} - 1}{a - 1} \approx \dfrac{(2a - 1)W}{2(a - 1)}$, потеряли один пакет, а значит $L \approx \dfrac{2(a - 1)}{(2a - 1)W}$

Так как мы всегда увеличиваемся в $a$ раз, то увеличивается вдвое всегда за одинаковое количество шагов ($\log_a 2$), а это ни от чего кроме параметров не зависит (мы последний раз делаем $a^{n} \cdot \ldots$, а первое как раз примерно 2), что и требовалось, так как время тоже будет одинаковым (кол-во шагов помножается на RTT)
